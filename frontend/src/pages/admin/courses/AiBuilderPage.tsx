 ...
 * AiBuilderPage.tsx
 *
 * Route entry point for the AI course building flow.
 * Mounted at: /teacher/classroom/:courseId/:unitId/ai-builder
 *
...
export default function AiBuilderPage() {
  ...
  const handleStartBuilding = useCallback(
    (builtOutline: CourseOutline, builtConfig: WizardConfig) => {
      /**
       * Navigate to the existing TeacherOnboardingPage builder phase.
       * We pass the outline + config through location state so the builder
       * can skip the setup steps and go straight to CourseBuildScreen.
       *
       * The route /admin/courses/builder already renders TeacherOnboardingPage
       * which reads location.state.outline / location.state.courseData
       * to bypass SetupFlow if present.
       */
      navigate('/admin/courses/builder', {
        state: {
          outline:     builtOutline,
          courseData:  {
            subject:           builtConfig.subject,
            level:             builtConfig.level,
            nativeLanguage:    builtConfig.nativeLanguage,
            unitCount:         builtConfig.unitCount,
            extraInstructions: builtConfig.extraInstructions,
          },
          uploadedFiles: [], // files already ingested via RAG at wizard step
          courseId:      courseId ? Number(courseId) : undefined,
          unitId:        unitId   ? Number(unitId)   : undefined,
        },
      });
    },
    [navigate, courseId, unitId],
  );

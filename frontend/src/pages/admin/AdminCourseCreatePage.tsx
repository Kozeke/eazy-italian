  ...
  const handleSave = async (publish: boolean = false) => {
    ...
    try {
      ...
      const normalizedSettings = Object.fromEntries(
        Object.entries(formData.settings).filter(([, value]) => value !== null && value !== undefined && value !== '')
      );
      ...
      if (formData.level) {
        courseData.level = formData.level as 'A1' | 'A2' | 'B1' | 'B2' | 'C1' | 'C2' | 'mixed';
      }
      if (publish) {
        courseData.status = 'published';
      } else if (formData.publish_at) {
        courseData.status = 'scheduled';
      }
      if (formData.publish_at) {
        courseData.publish_at = formData.publish_at;
      }
      if (formData.order_index !== 0) {
        courseData.order_index = formData.order_index;
      }
      if (formData.thumbnail_url.trim()) {
        courseData.thumbnail_url = formData.thumbnail_url.trim();
      }
      if (formData.duration_hours !== null) {
        courseData.duration_hours = formData.duration_hours;
      }
      if (formData.tags.length > 0) {
        courseData.tags = formData.tags;
      }
      if (formData.meta_title.trim()) {
        courseData.meta_title = formData.meta_title.trim();
      }
      if (formData.meta_description.trim()) {
        courseData.meta_description = formData.meta_description.trim();
      }
      if (formData.is_visible_to_students) {
        courseData.is_visible_to_students = true;
      }
      if (Object.keys(normalizedSettings).length > 0) {
        courseData.settings = normalizedSettings;
      }

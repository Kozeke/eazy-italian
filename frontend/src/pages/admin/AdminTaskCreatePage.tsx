...
export default function AdminTaskCreatePage() {
  ...
  const [gradingType, setGradingType] = useState<'automatic' | 'manual'>('automatic');
  ...
  const stepNumber = (field: 'maxScore' | 'maxAttempts', delta: number) => {
    if (field === 'maxScore') {
      setMaxScore(Math.max(1, Math.min(9999, maxScore + delta)));
    } else if (field === 'maxAttempts') {
      setMaxAttempts(Math.max(1, Math.min(99, (maxAttempts || 1) + delta)));
    }
  };
  ...
  const typeConfig: Record<string, { label: string; color: string }> = {
    writing: { label: 'Письмо', color: 'var(--teal)' },
    listening: { label: 'Аудирование', color: 'var(--gold)' },
      ...
      <div className="tf-page tf-root">
        <header className="tf-topbar">
          <div className="tf-topbar-left">
            ...
            <div className="tf-breadcrumb">
              ...
              <span className="current">Создать задание</span>
            </div>
          </div>
        ...
        <div className="tf-page-content">
          <div className="tf-page-header">
            <h1 className="tf-page-title">Создать <em>задание</em></h1>
          </div>
